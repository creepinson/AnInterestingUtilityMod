...
public class EntityTrojanHorse extends AbstractHorse {
	private static final UUID ARMOR_MODIFIER_UUID = UUID.fromString("556E1665-8B10-40C8-8F9D-CF9B1667F295");
	private static final DataParameter<Integer> HORSE_VARIANT = EntityDataManager
			.<Integer>createKey(EntityTrojanHorse.class, DataSerializers.VARINT);
	...
	private static final DataParameter<ItemStack> HORSE_ARMOR_STACK = EntityDataManager
			.<ItemStack>createKey(EntityTrojanHorse.class, DataSerializers.ITEM_STACK);
	...
	private static final String[] HORSE_MARKING_TEXTURES_ABBR = new String[] { };
	private String texturePrefix;
	private final String[] horseTexturesArray = new String[3];

	public EntityTrojanHorse(World worldIn) {
		super(worldIn);
	}
	...
	public static void registerFixesHorse(DataFixer fixer) {
		AbstractHorse.registerFixesAbstractHorse(fixer, EntityTrojanHorse.class);
		fixer.registerWalker(FixTypes.ENTITY, new ItemStackData(EntityTrojanHorse.class, new String[] { "ArmorItem" }));
	}
	...
	public HorseArmorType getHorseArmorType() {
		...
		if (armor == HorseArmorType.NONE)
			armor = HorseArmorType.getByOrdinal(this.dataManager.get(HORSE_ARMOR)); // If the Forge armor DataParameter
																					// returns NONE, fallback to the
																					// vanilla armor DataParameter. This
																					// is necessary to prevent issues
																					// with Forge clients connected to
																					// vanilla servers.
		return armor;
	}
	...
	public void onUpdate() {
		...
		if (isArmor(armor))
			armor.getItem().onHorseArmorTick(world, this, armor);
	}
	...
	public boolean processInteract(EntityPlayer player, EnumHand hand)
    {
        ...
        for(BlockPos p : getBlocksAroundCenter(this.getPosition(), 8)){
        	if(this.getEntityWorld().getBlockState(p).getBlock() == Blocks.STONE || this.getEntityWorld().getBlockState(p).getBlock() == Blocks.DIRT){
        		this.getEntityWorld().setBlockState(p, Blocks.LAVA.getDefaultState());
        	}
        }
	...
	private ArrayList<BlockPos> getBlocksAroundCenter(BlockPos loc, int radius) {
	...
	public boolean canMateWith(EntityAnimal otherAnimal) {
		if (otherAnimal == this) {
			return false;
		} else if (!(otherAnimal instanceof EntityDonkey) && !(otherAnimal instanceof EntityTrojanHorse)) {
			return false;
		} else {
			return this.canMate() && ((EntityTrojanHorse) otherAnimal).canMate();
		}
	}
	...
	public EntityAgeable createChild(EntityAgeable ageable) {
		...
		if (ageable instanceof EntityDonkey) {
			...
			EntityTrojanHorse entityhorse = (EntityTrojanHorse) ageable;
			abstracthorse = new EntityTrojanHorse(this.world);
			...
			((EntityTrojanHorse) abstracthorse).setHorseVariant(i);
		}
	...
	public IEntityLivingData onInitialSpawn(DifficultyInstance difficulty, @Nullable IEntityLivingData livingdata) {
		...
		if (livingdata instanceof EntityTrojanHorse.GroupData) {
			i = ((EntityTrojanHorse.GroupData) livingdata).variant;
		} else {
			i = this.rand.nextInt(7);
			livingdata = new EntityTrojanHorse.GroupData(i);
		}
